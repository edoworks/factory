...
class LifecycleContractTests(unittest.TestCase):
    ...
    def test_hosted_ci_bounds_storage_and_form_factor_checks(self):
        ...
        self.assertIn("Run 36184422124 reached the former 15-minute bound", workflow)
        self.assertRegex(
            workflow,
            r"(?m)^      - name: Verify reference app\n        #.*\n        timeout-minutes: 20$",
        )
        self.assertIn('form_factor: iPhone', workflow)
        self.assertIn('form_factor: iPad', workflow)
        package = workflow.index("- name: Package evidence")
        upload = workflow.index("- name: Upload evidence", package)
        package_block = workflow[package:upload]
        upload_block = workflow[upload:]
        self.assertLess(package, upload)
        self.assertRegex(package_block, r"(?m)^- name: Package evidence\n        if: always\(\)$")
        self.assertIn('RESULT="/tmp/MewsAndWoofs-${{ matrix.form_factor }}.xcresult"', package_block)
        self.assertIn('ARCHIVE="/tmp/MewsAndWoofs-${{ matrix.form_factor }}.xcresult.zip"', package_block)
        self.assertIn('ditto -c -k --sequesterRsrc --keepParent "$RESULT" "$ARCHIVE"', package_block)
        self.assertIn('mews-and-woofs-${{ matrix.form_factor }}-evidence', workflow)
        self.assertRegex(upload_block, r"(?m)^- name: Upload evidence\n        if: always\(\)$")
        self.assertIn("if-no-files-found: error", upload_block)
        self.assertIn('path: /tmp/MewsAndWoofs-${{ matrix.form_factor }}.xcresult.zip', upload_block)
        self.assertNotRegex(upload_block, r"(?m)^\s+path: .*\.xcresult$")
