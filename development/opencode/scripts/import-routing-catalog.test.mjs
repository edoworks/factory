...
import { buildRoutingSnapshot, importRoutingCatalog } from "./import-routing-catalog.mjs";
...
test("invalid import leaves the tracked catalog and manifest unchanged", async () => {
  ...
  const digest = createHash("sha256").update(catalogText).digest("hex");
  const manifestText = `${JSON.stringify({ schema_version: 1, files: { "model-routing/catalog.json": digest } }, null, 2)}\n`;
  await Promise.all([
    writeFile(join(routing, "catalog.json"), catalogText),
    writeFile(join(routing, "approved.json"), `${JSON.stringify(registry)}\n`),
    writeFile(join(routing, "policy.json"), `${JSON.stringify(policy)}\n`),
